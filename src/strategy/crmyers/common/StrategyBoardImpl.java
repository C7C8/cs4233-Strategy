...
public class StrategyBoardImpl implements strategy.Board {
	...
	/**
	 * Copy a given board of the given size to this one. Does not perform validity checking on its own since this
	 * board doesn't know what game it belongs to.
	...
	/**
	 * Simple helper to check bounds, since this is used a lot. Since the ultimate result of out-of-bounds would be
	 * throwing an exception anyways, to remove a whole lot of conditionals this function just throws an exception instead.
	 * @param row Row
	 * @param col Column
	...
	/**
	 * Helper to validate the board setup by making sure pieces are in the right places and have the correct count.
	 * Works by making sure that `startingPieceRows` rows on the top and bottom are filled in, counting pieces as it
	 * goes, then making sure the zone between is empty. Once that's done it checks against the refPieceCounts array
	 * to make sure that the specified number of pieces exist on the board for each color, no more, no less.
	...
	public boolean validateBoard(int[] refPieceCounts, int startingPieceRows) {
		...
		// Make sure the middle zone is clear
		for (int r = startingPieceRows; r < ROWS - startingPieceRows; r++) {
			for (int c = 0; c < COLS; c++) {
				if (pieces[r][c] != null)
					return false;
			}
	...
	/**
	 * Compare boards, determine if they're equal. Partly autogenerated.
	...
	/**
	 * Convert to a printable grid, useful for debugging.
